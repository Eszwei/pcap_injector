...
def process_command_line(argv):
    ...
    parser.add_option('-f', '--field',
                      help='Specify the field to exploit to contain the payload (i.e., TOS, TTL, ID, TIMING).',
...
def find_flows(pcap_to_read, number_packets):
    ...
    df_tcp = df.groupby(['ip.src', 'ip.dst', 'tcp.srcport', 'tcp.dstport', 'ip.proto']).size().to_frame(
        '#pkts').reset_index()
    df_udp = df.groupby(['ip.src', 'ip.dst', 'udp.srcport', 'udp.dstport', 'ip.proto']).size().to_frame(
        '#pkts').reset_index()
    df_icmp = df.groupby(['ip.src', 'ip.dst', 'ip.proto']).size().to_frame('#pkts').reset_index()
    ...
    df_final = df_tcp.append(df_udp, ignore_index=True).append(df_icmp, ignore_index=True)
    df_final = df_final[
        ['ip.src', 'ip.dst', 'tcp.srcport', 'tcp.dstport', 'udp.srcport', 'udp.dstport', 'ip.proto', '#pkts']]
    df_final = df_final.fillna('-')
    return df_final.loc[df_final['#pkts'] >= number_packets]


def inject(pcap, source, destination, src_port, dst_port, protocol, targeted_field, attack_in_chunks, attack):
    ...
    for x in range(len(pkts)):
        ...
        if ICMP in pkts[x] or TCP in pkts[x] or UDP in pkts[x]:
            # Search for the correct flow
            if (source == pkts[x][IP].src and destination == pkts[x][IP].dst and protocol == pkts[x][IP].proto) and \
                    (protocol == 1 or (src_port == pkts[x].sport and dst_port == pkts[x].dport)):
...
def flow_selection(flows, number):
    ...
    elif int(flows.loc[number]['ip.proto']) == 1:
        protocol = 1
        src_port = 0
        dst_port = 0
    else:
...
def write_to_csv(csv_file_name, filename, attack, field, src, dst, p_src, p_dst, proto, lengthb, lengthp):
    ...
    with open(csv_file_name, mode='a') as file:
        ...
        if not csv_exists:
            writer.writerow(['file name', 'attack', 'field', 'src', 'dst', 'p_src', 'p_dst', 'proto', 'length [packet]',
                             'length [bit]'])
...
if len(flows) > 0:
    ...
    while True:
        operation = input(
            "Choose the flow by its index (leave it blank for the first flow or 'r' for a random choice): ")
    ...
    print('-' * 25)
    print("Wireshark filter: ip.src == " + str(source) + " and ip.dst == " + str(
        destination) + " and tcp.srcport == " + str(src_port) + " and tcp.dstport == " + str(dst_port))
    resulting_pcap_file = inject(settings.pcap, source, destination, src_port, dst_port, protocol, settings.field,
                                 attack_in_chunks, settings.attack)
    write_to_csv('injected_flows.csv', settings.pcap, settings.attack, settings.field, source, destination, src_port,
                 dst_port, protocol, lengthp, lengthb)
